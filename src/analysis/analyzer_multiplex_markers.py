...
class AnalyzerMultiplexMarkers(Analyzer):
    ...
    def load(self) -> None:
        """
        Load the saved multiplexed_embeddings, paths, and labels into 
        self.features, self.labels, and self.paths.
        """
        output_folder_path = self.get_saving_folder(feature_type='multiplexed')
        logging.info(f"[load multiplex markers]: output_folder_path: {output_folder_path}")
        self.features = np.load(self._get_save_path(output_folder_path, "mp_emb.npy"))
        self.labels = np.load(self._get_save_path(output_folder_path, "labels.npy"))
        self.paths = np.load(self._get_save_path(output_folder_path, "paths.npy"))

    def save(self) -> None:
        """
        Save the multiplexed features, labels, and paths to .npy files.
        """
        output_folder_path = self.get_saving_folder(feature_type='multiplexed')
        os.makedirs(output_folder_path, exist_ok=True)
        logging.info(f"[save multiplex markers] Saving to {output_folder_path}")
        np.save(self._get_save_path(output_folder_path, "mp_emb.npy"), self.features)
        np.save(self._get_save_path(output_folder_path, "labels.npy"), self.labels)
        np.save(self._get_save_path(output_folder_path, "paths.npy"), self.paths)

    def _get_save_path(self, output_folder_path: str, file_type: str) -> str:
        return os.path.join(output_folder_path, f"{file_type}")
